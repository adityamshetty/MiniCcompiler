...
entry_t *create_entry(char *token, char *lexeme, int value, int yylineno, char *type)
{
    entry_t *newentry;
...
entry_t *insert(entry_t **hash_table_ptr, char *token, char *lexeme, char *data_type, int value, int lineno)
{
    ...
    newentry = create_entry(token, lexeme, value, lineno, data_type); // Create an entry using the <lexeme, token> pair
...
void display(entry_t **hash_table_ptr)
{
    ...
    for (i = 0; i < HASH_TABLE_SIZE; i++)
    {
        ...
        {
            printf(" %-20s %-20s %-20s %-20d \n", traverser->token, traverser->lexeme, traverser->data_type, traverser->line);
